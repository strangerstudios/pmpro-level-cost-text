...
<style>	
#template_reference {background: #FAFAFA; border: 1px solid #CCC; color: #666; padding: 5px; }
...
<table class="form-table">
	<tbody>
		<tr>
			...
			<td>
				<textarea name="level_cost_text" rows="4" style="width: 100%;"><?php echo esc_textarea(wp_unslash($level_cost_text));?></textarea>
...
{
	pmpro_saveCustomLevelCostText($level_id, wp_kses_post( $_REQUEST['level_cost_text'] ));			//add level cost text for this level			
}
	...
	<tr>
		<td>
			<tr>
				<th scope="row" valign="top"><label for="level_cost_text"><<?php _e("Level Cost Text", "pmpro-level-cost-text"); ?>:</label></th>
				<td>
					<textarea name="level_cost_text[]" rows="4" cols="50"><?php echo wp_kses_post($level_cost_text);?></textarea>
					<br /><small><?php _e("If completely blank, the default text generated by PMPro will be used.", "pmpro-level-cost-text"); ?></small>
				</td>
			</tr>
		</td>
	</tr> 
...
function pclct_pmpro_save_discount_code_level($code_id, $level_id)
{
	...
	$level_cost_text_a = wp_kses_post( $_REQUEST['level_cost_text'] );			//level_cost_text for levels checked
