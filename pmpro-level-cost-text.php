...
function pclct_pmpro_membership_level_after_other_settings()
{
	...
	?>
	<hr />
	<h2><?php esc_html_e("Custom Level Cost Text", "pmpro-level-cost-text"); ?></h2>
	<p>
		<?php echo sprintf(__('Override the default level cost using the available placeholders or custom text. Make sure the prices in this text match your settings above. You can modify the format of the default text in %s', 'pmpro-level-cost-text'), '<a href="' . esc_url( admin_url('admin.php?page=pmpro-advancedsettings') ) . '">' . esc_html__('Advanced Settings', 'pmpro-level-cost-text') . '.</a>');?>
		<?php
		$level_cost_text_link = '<a title="' . esc_attr__( 'Custom Level Cost Text Add On Documentation', 'pmpro-level-cost-text' ) . '" target="_blank" rel="nofollow noopener" href="https://www.paidmembershipspro.com/add-ons/pmpro-custom-level-cost-text/?utm_source=plugin&utm_medium=pmpro-level-cost-text&utm_campaign=add-ons">' . esc_html__( 'Custom Level Cost Text Add On', 'pmpro-level-cost-text' ) . '</a>';
		printf( esc_html__( 'Learn more about the %s.', 'pmpro-level-cost-text' ), $level_cost_text_link ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		?>
	</p>
